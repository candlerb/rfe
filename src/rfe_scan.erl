...
-type cont_fun() :: fun((char_spec(), #rfe_scan{}, line(), column(),
                         tokens(), any()) -> any()).
-opaque return_cont() :: {string(), column(), tokens(), line(), 
                          #rfe_scan{}, cont_fun(), any()}.
...
scan1([$\s|Cs], St, Line, Col, Toks) when St#rfe_scan.ws ->
...
scan1([$\%|Cs], St, Line, Col, Toks) when not St#rfe_scan.comment ->
        ...
        {Wcs,Cs} ->
            case catch list_to_atom(Wcs) of
                Name when is_atom(Name) ->
                    case (St#rfe_scan.resword_fun)(Name) of
                        true -> 
...
scan_string0(Cs, #rfe_scan{text=false}, Line, no_col=Col, Q, [], Wcs, Uni) ->
    scan_string_no_col(Cs, Line, Col, Q, Wcs, Uni);
scan_string0(Cs, #rfe_scan{text=true}, Line, no_col=Col, Q, Str, Wcs, Uni) ->
...
tok2(Cs, #rfe_scan{text = false}=St, Line, no_col=Col, Toks, _Wcs, P) ->
...
tok2(Cs, #rfe_scan{text = false}=St, Line, no_col=Col, Toks, _Wcs, P, _N) ->
    scan1(Cs, St, Line, Col, [{P,Line}|Toks]);
tok2(Cs, St, Line, Col, Toks, Wcs, P, N) ->
    Attrs = attributes(Line, Col, St, Wcs),
    scan1(Cs, St, Line, incr_column(Col, N), [{P,Attrs}|Toks]).

tok3(Cs, #rfe_scan{text = false}=St, Line, no_col=Col, Toks, Item, _S, Sym) ->
...
tok3(Cs, #rfe_scan{text = false}=St, Line, no_col=Col, Toks, Item,
     _String, Sym, _Length) ->
    scan1(Cs, St, Line, Col, [{Item,Line,Sym}|Toks]);
...
attributes(Line, no_col, #rfe_scan{text = false}, _String) ->
    Line;
attributes(Line, no_col, #rfe_scan{text = true}, String) ->
    [{line,Line},{text,String}];
attributes(Line, Col, #rfe_scan{text = false}, _String) ->
    {Line,Col};
attributes(Line, Col, #rfe_scan{text = true}, String) ->
