...
%% Modified by Brian Candler 2010.
%% TODO: Point to existing functions in io where they are unchanged.
%% 
...
-export([scan_rfe_exprs/1,scan_rfe_exprs/2,scan_rfe_exprs/3,
	 scan_rfe_form/1,scan_rfe_form/2,scan_rfe_form/3,
	 parse_rfe_exprs/1,parse_rfe_exprs/2,parse_rfe_exprs/3,
	 parse_rfe_form/1,parse_rfe_form/2,parse_rfe_form/3]).
...
%% XXX: Some uses of line() in this file may need to read rfe_scan:location()
...
-spec read(prompt()) -> 
    {'ok', term()} | 'eof' | {'error', rfe_scan:error_info()}.

% Read does not use get_until as rfe_scan does not work with unicode
% XXX:PaN fixme?
read(Prompt) ->
...
read(Io, Prompt) ->
    case request(Io, {get_until,unicode,Prompt,rfe_scan,tokens,[1]}) of
	{ok,Toks,_EndLine} ->
	    rfe_parse:parse_term(Toks);
...
-spec read(device(), prompt(), line()) ->
    ...
    {'error', rfe_scan:error_info(), line()}.

read(Io, Prompt, StartLine) when is_integer(StartLine) ->
    case request(Io, {get_until,unicode,Prompt,rfe_scan,tokens,[StartLine]}) of
	{ok,Toks,EndLine} ->
            case rfe_parse:parse_term(Toks) of
...
-spec scan_rfe_exprs(prompt()) -> rfe_scan:tokens_result().
 
scan_rfe_exprs(Prompt) ->
    scan_rfe_exprs(default_input(), Prompt, 1).

-spec scan_rfe_exprs(device(), prompt()) -> rfe_scan:tokens_result().

scan_rfe_exprs(Io, Prompt) ->
    scan_rfe_exprs(Io, Prompt, 1).

-spec scan_rfe_exprs(device(), prompt(), line()) -> rfe_scan:tokens_result().

scan_rfe_exprs(Io, Prompt, Pos0) ->
    request(Io, {get_until,unicode,Prompt,rfe_scan,tokens,[Pos0]}).

-spec scan_rfe_form(prompt()) -> rfe_scan:tokens_result().

scan_rfe_form(Prompt) ->
    scan_rfe_form(default_input(), Prompt, 1).

-spec scan_rfe_form(device(), prompt()) -> rfe_scan:tokens_result().

scan_rfe_form(Io, Prompt) ->
    scan_rfe_form(Io, Prompt, 1).

-spec scan_rfe_form(device(), prompt(), line()) -> rfe_scan:tokens_result().

scan_rfe_form(Io, Prompt, Pos0) ->
    request(Io, {get_until,unicode,Prompt,rfe_scan,tokens,[Pos0]}).

%% Parsing Erlang code.

-type rfe_parse_expr_list() :: [_]. %% XXX: should be imported from rfe_parse

-type parse_ret() :: {'ok', rfe_parse_expr_list(), line()}
                   | {'eof', line()}
                   | {'error', rfe_scan:error_info(), line()}.

-spec parse_rfe_exprs(prompt()) -> parse_ret().

parse_rfe_exprs(Prompt) ->
    parse_rfe_exprs(default_input(), Prompt, 1).

-spec parse_rfe_exprs(device(), prompt()) -> parse_ret().

parse_rfe_exprs(Io, Prompt) ->
    parse_rfe_exprs(Io, Prompt, 1).

-spec parse_rfe_exprs(device(), prompt(), line()) -> parse_ret().

parse_rfe_exprs(Io, Prompt, Pos0) ->
    case request(Io, {get_until,unicode,Prompt,rfe_scan,tokens,[Pos0]}) of
	{ok,Toks,EndPos} ->
	    case rfe_parse:parse_exprs(Toks) of
...
-type rfe_parse_absform() :: _. %% XXX: should be imported from rfe_parse

-type parse_form_ret() :: {'ok', rfe_parse_absform(), line()}
                        | {'eof', line()}
                        | {'error', rfe_scan:error_info(), line()}.

-spec parse_rfe_form(prompt()) -> parse_form_ret().

parse_rfe_form(Prompt) ->
    parse_rfe_form(default_input(), Prompt, 1).

-spec parse_rfe_form(device(), prompt()) -> parse_form_ret().

parse_rfe_form(Io, Prompt) ->
    parse_rfe_form(Io, Prompt, 1).

-spec parse_rfe_form(device(), prompt(), line()) -> parse_form_ret().

parse_rfe_form(Io, Prompt, Pos0) ->
    case request(Io, {get_until,unicode,Prompt,rfe_scan,tokens,[Pos0]}) of
	{ok,Toks,EndPos} ->
	    case rfe_parse:parse_form(Toks) of
		{ok,Exprs} -> {ok,Exprs,EndPos};
		{error,E} -> {error,E,EndPos}
	    end;
	Other ->
